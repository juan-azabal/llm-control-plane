...
- **NeMo Guardrails** — semantic policy engine (Colang 1.0 DSL)
- **Ollama + Llama 3.2 3B** — local judge LLM (no data leaves the network)
...
1. **Sensitive data never reaches the cloud.** Layer 1 (deterministic) and Layer 2 (semantic) both run fully locally. Only clean, scrubbed requests reach OpenAI.
...
guardrails/<tenant>/        ← NeMo configs per tenant. See Colang section below.
  config.yml                ← NeMo engine config (models, rails enabled)
  config.co                 ← Colang flows (the actual policy rules)
  kb/                       ← Knowledge base for fact-checking (support only)
```
...
NeMo runs the configured Colang flows, then either:
- Returns the original response (passed) — `response.activated_rails` is empty or all "executed"
- Returns a refusal message (blocked) — `response.activated_rails` contains rails with `status: "blocked"`

The bridge also does heuristic phrase matching for cases where NeMo soft-blocks without setting the status field.
...
- **Don't restart NeMo mid-request.** NeMo caches rail configs on startup. If you change a `.co` file, restart the NeMo container.
...
1. **3B model judge quality.** Llama 3.2 3B can miss novel jailbreaks or produce false positives on legitimate queries. See `docs/decisions/010-jailbreak-detection.md`.

2. **NeMo latency.** Each request that hits NeMo adds 1–10 seconds (3B model inference on CPU). On GPU this drops to <1s.

3. **Fact-checking accuracy.** `self_check_facts` with a 3B model is probabilistic. The knowledge base must be kept simple.

4. **No streaming support.** LiteLLM streaming with custom guardrails has limitations. Streaming is disabled for guarded requests.

5. **Single-node only.** No horizontal scaling designed for this demo. LiteLLM supports Redis for distributed deployments (ADR-002).
...
The NeMo image builds from `python:3.11-slim` and installs `nemoguardrails` plus `langchain-ollama` for Ollama integration.
